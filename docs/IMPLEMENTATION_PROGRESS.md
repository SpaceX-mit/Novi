...
- [ ] 将 Workspace Composer 改造为“每轮对话持续完善成果”：每轮按当前问题检索更多权威资料、把可保留内容写入 Workspace knowledge，并生成新的不可变 `llm-wiki.md`/LLM Wiki Artifact 版本。当前 Composer 仍是独立对话 Harness，只保存消息和工具 provenance，不创建 Artifact。
...
- 2026-08-17：补齐 Agent 正式论文工具：Customize/Tools 从 3 个扩展为 5 个内置工具，新增 `paper_search`（OpenAlex、arXiv、Crossref、Semantic Scholar、IEEE/ACM/Springer 学术目录）和 `paper_fetch`（DOI、arXiv、公开 URL）。论文获取复用 DNS/SSRF、重定向、超时、8 MB 与内容哈希边界，明确区分 `metadata-only`、`abstract-only`、`public-page-text`、`public-full-text`、`unavailable`；只有成功下载并解析的公开 PDF 标记全文，模型观察最多 12000 字符且记录是否截断。两项工具只在实时来源开启并取得来源额度时进入 Agent registry，调用沿用最多 6 次限制和 Job/Session/Artifact provenance；搜索结果或不可访问正文不会冒充已验证全文。验证：`npm test` 84 passed + 1 PostgreSQL 条件跳过，`npm run check` 49 modules，`npm run openapi-check` 47 paths / 59 operations，1360x900 与 390x844 `npm run browser-smoke`、`npm run release-check`、`git diff --check` 通过。真实学术服务速率、付费访问和内容质量仍属于生产外部门禁。新增需求“每轮对话持续完善 Markdown/Wiki 并丰富知识库”已记录为下一独立功能，尚未宣称完成。
