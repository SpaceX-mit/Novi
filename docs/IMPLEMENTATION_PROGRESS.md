...
| Agent Runtime | 意图路由的 Workflow、ReAct、Plan & Execute、Supervisor 四模式 LangGraph.js StateGraph；Goal Architect → Goal 驱动 Reference Discovery → Research/Knowledge/Writing/Review specialist → 必经 LLM Wiki Finalizer；Composer 与直接生成共享完整图，Composer 以前一成果和累计来源为基线；controller/阶段 fallback 可运行中切换模式，默认最多 16 个 Specialist 执行、单一职责最多 3 次尝试和 24 次工具调用，预算可配置并写入 runtime；字段/形状校验、token、语言、参考 provenance、模式历史、计划和 Job 进度可追溯；运行事件记录模型请求/回复、Agent 路由和阶段 | 本地 OpenAI-compatible HTTP 验证四种成果模式、7 节点 Goal/Reference/专家协作/Finalizer、两轮累积来源与 Wiki refinement、检索失败退款、提前 finish 仍经 Finalizer、ReAct → Plan & Execute 运行中切换及工具循环；`npm test` 85 passed + 1 PostgreSQL 条件跳过，验证默认 24 次工具预算和 runtime provenance；Chromium 验证实时事件时间线和历史详情 |
...
| 自动化门禁 | 测试、语法、OpenAPI、供应商/存储契约、浏览器、SBOM、依赖与镜像扫描、release-check | 2026-08-18：`npm test` 85 passed + 1 PostgreSQL 条件跳过；51 个 JavaScript 模块语法通过；OpenAPI 48 paths / 60 operations；browser smoke、Provider/存储契约和 release-check 通过；凭据形状扫描与 `git diff --check` 通过 |
