...
| FR-37 | 持久 Agent Session | 创建项目时生成默认 Session；可按项目列出、新建、查看和删除空闲 Session。同步/异步生成接受 `sessionId`，保存用户/助手消息、active run 模式/阶段/进度、Job 与 Artifact 关联；每次运行保存最多 100 条有界事件，覆盖模型请求/回复、Agent 阶段/路由、参考检索、工具输入输出和 Artifact 提交，并在完成/失败消息中固化；Composer 每轮创建 `refine` Job，以上一 Artifact 和累积来源为基线生成新版本，并把 `llm-wiki.md` 索引为 `agent-wiki` Workspace knowledge；Session 按 tenant+project 隔离并随项目/账户删除，运行中删除返回 409，服务重启把中断运行写为失败并解除占用 | `src/agent-sessions.mjs`, `/api/projects/:id/sessions*`, `agentSessions` |
| FR-38 | Conversation Session 工作区 | 创建项目后直接进入默认 Session；左栏切换/新建/删除 Session，中栏显示持久研究迭代并通过 composer 选择 auto 或四种执行模式及 Wiki 语言；运行中实时刷新可展开 Agent 事件时间线，完成后在助手消息中保留请求、回复、工具和产物详情。每轮消息执行 Goal → Reference → Specialist → Finalizer、合并权威来源、创建 Artifact/Markdown 并丰富知识库。右栏在 Files、LLM Wiki、Document 间查看文件、成果和片段。Generate now 进入当前 Session；页面重开可恢复 active Job 轮询；viewer 只读；桌面和 390px 移动宽度无溢出 | `public/app.js`, `public/styles.css`, `scripts/browser-smoke.mjs` |
...
| FR-40 | 声明式 Agent Plugins | owner/admin 可把现有 Skills 和已授权 Tool/MCP 组合为最多 10 个版本化 manifest；每次选择最多 2 个，工具引用与运行 registry 取交集。Plugin 不下载/执行代码、不携带凭据或授予权限；Job/Session/Artifact 保存组合与 manifest 哈希 | `src/plugin-runtime.mjs`, `/api/agent/plugins`, `Customize/Plugins` |
...
- `POST /api/projects/:id/sessions/:sessionId/messages`：提交 `{prompt,mode,language}`，异步运行累积式 Wiki refinement；完成后追加关联新 Artifact 的助手消息并索引 Markdown。
