...
- `src/agent-runtime.mjs`：LangGraph.js 自适应有向图；Router 强制先执行 Goal 和无模型调用的 Reference Discovery，再进入固定流水线、ReAct controller、Planner 或 Supervisor。Reference 查询只来自已完成 Goal 的 question/domain/outcome/scope，默认展开为 5 个独立 facet，返回来源仍由 Novi 连接器和 evidence 层控制。controller 可切换模式，Specialist/Finalizer 质量失败会在阶段尝试预算内重新进入同一节点，耗尽后才记录 fallback。每个 Specialist 建立含目标语言约束和领域质量契约的有界 prompt，使用 `model.stream()` 聚合增量并通过 `onModel` 回调写入 Session/Job 的 `streaming` 事件；流结束后才校验 JSON、字段形状和最小内容深度，只有校验成功的字段合入状态。reasoning 模型返回的 `<think>`/`<analysis>` 与 Markdown JSON fence 会先被分离，解析器按平衡括号提取完整 JSON；仅当前阶段拥有且通过 schema 的字段可以合入状态。首个有效 token、流式空闲和阶段总时长分别受 `NOVI_LLM_TIMEOUT_MS`、`NOVI_LLM_STREAM_IDLE_TIMEOUT_MS`、`NOVI_LLM_STAGE_MAX_MS` 保护。收到内容后的解析/校验错误记录为 `LLM response rejected`，与网络/超时的 `LLM request failed` 区分。默认每次生成最多 24 次工具调用、16 次 Specialist 执行、单一职责 3 次尝试，生成图递归预算自动按配置扩展；预算可由 `NOVI_AGENT_*` 环境变量覆盖并固化到 runtime。取消信号终止整条工作流。
...
- `src/agent-tools.mjs`：把内置 workspace read/write、项目文件 read/search/write/patch、持久 memory、Skill list/view/manage、web search、paper search/fetch、自定义 HTTP 和已启用 MCP 工具合并为当次运行的受控注册表；文件路径统一规范化并限制在租户/项目虚拟文件树，memory 按 tenant+project+key 隔离且限制 200 条，Skill manage 重新执行 owner/admin RBAC 并记录审计。运行时执行输入校验、租户/项目/来源额度边界、取消检查和默认 24 次（可配置 6–64）调用上限，并把有界 observation/provenance 返回 LangGraph tool node。
- `src/agent-exec-tools.mjs`：实现默认双重关闭的 `terminal`/`exec`。每次调用把项目虚拟文件树物化到权限受限的临时目录，使用最小环境、程序 allowlist、1–30 秒超时和 64 KB 合并输出；结束后只接受最多 500 个、总计 2 MB、单文件 200 KB 的 UTF-8 常规文件，拒绝符号链接/二进制并重新索引 Workspace knowledge。`terminal` 是经命令段校验的一次性 shell，`exec` 不经过 shell。它不是生产级 OS/网络沙箱，生产启用仍需要外部容器/VM/seccomp 门禁。
...
异步 Job 通过 `GET /api/jobs/:id/events` 提供同源 SSE 快照；Web Session 首选 SSE，浏览器不支持或连接中断时回退 `/api/jobs/:id` 轮询。每个模型阶段将流式增量写入同一 Job/Session 事件 ID，UI 可在完整响应前显示 `streaming` 文本；聚合响应必须通过阶段 JSON/schema 校验后才会更新成果并提交 Artifact。
