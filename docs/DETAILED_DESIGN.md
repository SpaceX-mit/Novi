...
- `src/agent-tools.mjs`：把内置 workspace read/write、项目文件 read/search/write/patch、web search、paper search/fetch、自定义 HTTP 和已启用 MCP 工具合并为当次运行的受控注册表；文件路径统一规范化并限制在租户/项目虚拟文件树，拒绝绝对路径、穿越和 200 KB 以上内容，写入后同步重建 `workspace-file` knowledge 索引。运行时执行输入校验、租户/项目/来源额度边界、取消检查和最多 6 次调用的硬上限，并把有界 observation/provenance 返回 LangGraph tool node。
