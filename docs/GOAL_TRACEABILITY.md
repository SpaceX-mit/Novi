...
| 专家协作与 Agent 边界 | Goal 先生成领域专家团队；Goal 完成后按多个 facet 搜索论文、GitHub、Web；Research、Knowledge、Writing、Review 共享前序产物，Finalizer 形成最终 Wiki | `src/agent-runtime.mjs` 的 Goal/Reference Discovery/四 specialist/Finalizer 图节点；Job/Session 暴露每个 query 状态；阶段质量失败有界重试；Artifact 保存 7 节点 provenance | 已实现；真实检索受 `NOVI_LIVE_SOURCES`、额度和验证门禁控制，真实供应商质量仍需人工评测 |
