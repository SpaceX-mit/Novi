...
| 软件规格 | `docs/SRS.md` | FR-01 至 FR-34、NFR-01 至 NFR-18 均有验收条件和实现位置 | 已文档化，自动化覆盖见第 2 节 |
...
| API 契约 | `openapi.yaml` | `npm run openapi-check` 通过 OpenAPI 3.1 schema、引用、35 paths/40 operations | 已验证 |
| Web UI | `public/` | 创建/生成/指定版本导出、历史选择/逐节比较、来源搜索、静态或 Browser Agent JS 渲染知识导入、浏览/语义检索/单文档删除、RAG 上下文核查、变化驱动的持续更新、组织切换、工作空间删除；Practice Lab、Deep Research Wiki/Graph/SOTA、Paper Gap & Novelty/Sources、IEEE/ACM 下载、套餐定价与真实 checkout 入口 | Chromium smoke 已验证知识检索、生成、删除与历史 excerpt 保留闭环、两版本比较、来源变化自动生成第 3 个不可变版本、差异/更新状态、SVG、研究套件、论文缺口和出版模板；Browser Agent 导入另由 HTTP 运行时测试验证；1360×900 两个知识视图已人工检查 |
| 生成工作流 | `src/engine.mjs`、不可变 artifact | 有界 Research → Knowledge → Writing → Review 四阶段流水线；每阶段保存责任、完成状态和实际输出计数，Markdown 导出 provenance；不允许 Agent 间自由对话 | 已实现并在领域/HTTP 测试中验证 |
...
| 门禁 | 验证范围 | 最近结果 |
| --- | --- | --- |
| `npm run check` | server、src、public、desktop、scripts、test 全部 JS 语法 | 38 modules 通过 |
| `npm test` | 领域引擎、HTTP、认证/RBAC、配额、支付、OIDC、摄取、语义检索/RAG 安全、删除/生成竞态、来源 diff/自动更新/重试/取消、恢复、投影、出版目录、Browser Agent、MCP 和三条产品输出 | 66 passed，1 个 PostgreSQL 条件跳过 |
...
| `npm run openapi-check` | OpenAPI 3.1 schema 和引用 | 35 paths、40 operations 通过 |
...
| `npm run sbom-check` | 锁文件 CycloneDX/SPDX、生产依赖边界、许可证元数据完整性 | CycloneDX 1.5：320 components，runtime 27；SPDX 2.3：321 packages；通过 |
