...
function renderAgentMessage(message) {
  ...
  const intakeOptions = !isUser && message.kind === 'intake' && message.runtime?.status === 'incomplete' && Array.isArray(message.runtime.options)
    ? message.runtime.options.slice(0, 5).map((option) => `<button type="button" class="intake-option" data-intake-option="${escapeHtml(option.id)}" title="${escapeHtml(option.description || option.label)}"><b>${escapeHtml(option.label)}</b><small>${escapeHtml(option.description || '')}</small></button>`).join('')
    : '';
  return `<article class="agent-message ${isUser ? 'user' : 'assistant'} ${message.kind || 'message'}"><div class="message-author"><b>${isUser ? 'You' : 'Novi'}</b><span>${escapeHtml(meta)}</span></div><p>${escapeHtml(message.content)}</p>${intakeOptions ? `<div class="intake-options" aria-label="Research direction options">${intakeOptions}</div>` : ''}${plugins ? `<div class="message-plugins">${plugins}</div>` : ''}${skills ? `<div class="message-skills">${skills}</div>` : ''}${tools ? `<div class="message-tools">${tools}</div>` : ''}${renderRunEvents(message.runEvents, 'Run details')}${message.artifactId ? `<button class="message-artifact" data-artifact-id="${escapeHtml(message.artifactId)}">Open generated artifact</button>` : ''}</article>`;
}
...
function renderWorkspace(project, selected = state.activeTab) {
  ...
  $$('[data-intake-option]').forEach((button) => button.addEventListener('click', () => sendAgentMessage(project.id, { prompt: button.dataset.intakeOption, mode: $('#agent-mode')?.value || 'auto', language: $('#wiki-language')?.value || state.composerLanguage || 'zh-CN' })));
