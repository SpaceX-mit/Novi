...
try {
  ...
  const initialSessionUi = await evaluate(`({ sessions: document.querySelectorAll('.session-item').length, modes: [...document.querySelectorAll('#agent-mode option')].map((option) => option.value), languages: [...document.querySelectorAll('#wiki-language option')].map((option) => option.value), language: document.querySelector('#wiki-language').value, tabs: [...document.querySelectorAll('[data-workspace-panel]')].map((button) => button.textContent.trim()), activePanel: document.querySelector('[data-workspace-panel].active')?.dataset.workspacePanel, welcome: document.querySelector('.agent-message.welcome').textContent })`);
  if (initialSessionUi.sessions !== 1 || JSON.stringify(initialSessionUi.modes) !== JSON.stringify(['auto', 'workflow', 'react', 'plan-execute', 'supervisor']) || initialSessionUi.languages.length !== 8 || initialSessionUi.language !== 'en' || JSON.stringify(initialSessionUi.tabs) !== JSON.stringify(['Chat', 'Files', 'LLM Wiki', 'Document']) || initialSessionUi.activePanel !== 'chat' || !initialSessionUi.welcome.includes('Workspace ready')) throw new Error(`Initial Agent Session UI is incorrect: ${JSON.stringify(initialSessionUi)}`);
  const preservedComposer = await evaluate(`(async () => { const prompt = document.querySelector('#agent-prompt'); const mode = document.querySelector('#agent-mode'); prompt.value = 'Preserve this draft'; prompt.dispatchEvent(new Event('input', { bubbles: true })); mode.value = 'plan-execute'; mode.dispatchEvent(new Event('change', { bubbles: true })); document.querySelector('[data-workspace-panel="files"]').click(); const ui = await import('/app.js'); return { prompt: ui.state.composerDraft, mode: ui.state.composerMode, conversationHidden: !document.querySelector('.conversation-panel'), filesPanel: document.querySelector('[data-workspace-panel="files"]')?.classList.contains('active'), filesContent: !!document.querySelector('.workspace-context-panel') }; })()`);
  if (preservedComposer.prompt !== 'Preserve this draft' || preservedComposer.mode !== 'plan-execute' || !preservedComposer.conversationHidden || !preservedComposer.filesPanel || !preservedComposer.filesContent) throw new Error(`Composer state was lost while switching workspace tabs: ${JSON.stringify(preservedComposer)}`);
  ...
  await evaluate(`(() => { document.querySelector('[data-workspace-panel="chat"]').click(); const prompt = document.querySelector('#agent-prompt'); const mode = document.querySelector('#agent-mode'); prompt.value = 'Research authoritative sources and improve this Wiki'; prompt.dispatchEvent(new Event('input', { bubbles: true })); mode.value = 'workflow'; mode.dispatchEvent(new Event('change', { bubbles: true })); document.querySelector('#agent-composer').requestSubmit(); })()`);
  await waitFor(`document.querySelectorAll('.agent-message').length === 3 && document.querySelector('.agent-message.intake') !== null`);
  await evaluate(`(() => { const prompt = document.querySelector('#agent-prompt'); prompt.value = '确认生成'; prompt.dispatchEvent(new Event('input', { bubbles: true })); document.querySelector('#agent-composer').requestSubmit(); })()`);
  ...
  const liveTrace = await evaluate(`({ events: document.querySelectorAll('.live-events .run-event').length, request: document.querySelector('.live-events').textContent.includes('Request sent to LLM'), streaming: document.querySelector('.live-events').textContent.includes('streaming'), streamingOpen: document.querySelector('.live-events .run-event.streaming')?.open === true })`);
  if (liveTrace.events < 2 || !liveTrace.request || !liveTrace.streaming || !liveTrace.streamingOpen) throw new Error(`Live Agent run timeline is incomplete or the streaming LLM response is collapsed: ${JSON.stringify(liveTrace)}`);
  await waitFor(`document.querySelector('.message-artifact') !== null`, 15_000);
  const conversationUi = await evaluate(`(async () => { const projects = await (await fetch('/api/projects')).json(); const project = projects.projects.find((item) => item.title === 'Browser smoke workspace'); const knowledge = await (await fetch('/api/projects/' + encodeURIComponent(project.id) + '/knowledge')).json(); const last = document.querySelector('.agent-message:last-child'); const responses = [...last.querySelectorAll('.run-event')].filter((event) => event.querySelector('.run-event-type')?.textContent === 'model-response'); return { artifacts: project.artifacts.length, artifactLinks: document.querySelectorAll('.message-artifact').length, lastKind: last.className, wikiDocuments: knowledge.documents.filter((document) => document.sourceKind === 'agent-wiki').length, runEvents: last.querySelectorAll('.run-event').length, responseCount: responses.length, openResponses: responses.filter((event) => event.open).length, errors: last.querySelectorAll('.run-event-error').length, trace: last.textContent, details: [...last.querySelectorAll('.run-event pre')].map((node) => node.textContent).join('\\n') }; })()`);
  if (conversationUi.artifacts !== 1 || conversationUi.artifactLinks !== 1 || !conversationUi.lastKind.includes('artifact') || conversationUi.wikiDocuments !== 7 || conversationUi.runEvents < 10 || conversationUi.responseCount < 1 || conversationUi.openResponses !== conversationUi.responseCount || conversationUi.errors !== 0 || !conversationUi.trace.includes('LLM response') || !conversationUi.trace.includes('Artifact saved') || !conversationUi.details.includes('Browser Harness response')) throw new Error(`Composer did not create an auditable cumulative Wiki artifact with expanded LLM responses: ${JSON.stringify(conversationUi)}`);
  await evaluate(`fetch('/api/llm/provider', { method: 'DELETE' })`);
  await evaluate(`(() => { const prompt = document.querySelector('#agent-prompt'); const mode = document.querySelector('#agent-mode'); prompt.value = ''; prompt.dispatchEvent(new Event('input', { bubbles: true })); mode.value = 'auto'; mode.dispatchEvent(new Event('change', { bubbles: true })); document.querySelector('[data-workspace-panel="chat"]').click(); })()`);
  ...
  await evaluate(`document.querySelector('[data-workspace-panel="chat"]').click()`);
  await waitFor(`document.querySelector('.message-artifact') !== null`);
  ...
  if (!completedSessionUi.roles.includes('Novi') || !completedSessionUi.artifactLink.includes('Open generated artifact') || completedSessionUi.running) throw new Error(`Completed Agent Session UI is incorrect: ${JSON.stringify(completedSessionUi)}`);
  await evaluate(`document.querySelector('[data-workspace-panel="files"]').click()`);
  ...
  await evaluate(`document.querySelector('[data-workspace-panel="wiki"]').click()`);
  ...
  if (!viewerUi.createHidden || !viewerUi.providerHidden || !viewerUi.customizeHidden || viewerUi.generate || viewerUi.removeWorkspace || viewerUi.pin || viewerUi.ingest || viewerUi.refresh || viewerUi.watch || viewerUi.exportMarkdown === false || !viewerUi.conversation || viewerUi.composer || viewerUi.newSession || viewerUi.deleteSession) throw new Error(`Viewer UI capabilities are incorrect: ${JSON.stringify(viewerUi)}`);
  ...
  await evaluate(`document.querySelector('[data-workspace-panel="wiki"]').click(); document.querySelector('#knowledge-library').click()`);
  ...
  await waitFor(`document.querySelector('[data-workspace-panel="wiki"]') !== null`);
  await evaluate(`document.querySelector('[data-workspace-panel="wiki"]').click()`);
