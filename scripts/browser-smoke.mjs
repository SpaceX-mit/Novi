...
try {
  ...
  const viewerUi = await evaluate(`(async () => { const ui = await import('/app.js'); ui.state.role = 'viewer'; ui.applyRoleCapabilities(); ui.renderWorkspace(ui.state.activeProject); const result = { createHidden: document.querySelector('#new-project').hidden, generate: !!document.querySelector('#generate'), removeWorkspace: !!document.querySelector('#delete-workspace'), pin: !!document.querySelector('#pin-workspace'), ingest: !!document.querySelector('#ingest-document'), refresh: !!document.querySelector('#refresh-sources'), watch: !!document.querySelector('#toggle-watch'), browse: !!document.querySelector('#knowledge-library'), exportMarkdown: !!document.querySelector('#export-md') }; ui.state.role = 'owner'; ui.applyRoleCapabilities(); ui.renderWorkspace(ui.state.activeProject); return result; })()`);
  if (!viewerUi.createHidden || viewerUi.generate || viewerUi.removeWorkspace || viewerUi.pin || viewerUi.ingest || viewerUi.refresh || viewerUi.watch || !viewerUi.browse || !viewerUi.exportMarkdown) throw new Error(`Viewer UI capabilities are incorrect: ${JSON.stringify(viewerUi)}`);
