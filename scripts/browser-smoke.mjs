...
try {
  ...
  const conversationUi = await evaluate(`(async () => { const projects = await (await fetch('/api/projects')).json(); const project = projects.projects.find((item) => item.title === 'Browser smoke workspace'); const knowledge = await (await fetch('/api/projects/' + encodeURIComponent(project.id) + '/knowledge')).json(); const last = document.querySelector('.agent-message:last-child'); return { artifacts: project.artifacts.length, artifactLinks: document.querySelectorAll('.message-artifact').length, lastKind: last.className, wikiDocuments: knowledge.documents.filter((document) => document.sourceKind === 'agent-wiki').length, runEvents: last.querySelectorAll('.run-event').length, errors: last.querySelectorAll('.run-event-error').length, trace: last.textContent, details: [...last.querySelectorAll('.run-event pre')].map((node) => node.textContent).join('\\n') }; })()`);
  if (conversationUi.artifacts !== 1 || conversationUi.artifactLinks !== 1 || !conversationUi.lastKind.includes('artifact') || conversationUi.wikiDocuments !== 1 || conversationUi.runEvents < 10 || conversationUi.errors !== 0 || !conversationUi.trace.includes('LLM response') || !conversationUi.trace.includes('Artifact saved') || !conversationUi.details.includes('Browser Harness response')) throw new Error(`Composer did not create an auditable cumulative Wiki artifact: ${JSON.stringify(conversationUi)}`);
