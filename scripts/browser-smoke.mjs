...
try {
  ...
  await waitFor(`document.querySelector('#view-customize').classList.contains('active-view') && document.querySelectorAll('[data-builtin-tool]').length === 5`);
  ...
  if (JSON.stringify(toolUi.tabs) !== JSON.stringify(['Tools', 'MCP', 'Skills', 'Plugins']) || JSON.stringify(toolUi.builtins) !== JSON.stringify(['workspace_read', 'workspace_write', 'web_search', 'paper_search', 'paper_fetch']) || !toolUi.configurable || toolUi.overflow) throw new Error(`Customize tools UI is incorrect: ${JSON.stringify(toolUi)}`);
