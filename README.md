...
Web 中保存的租户配置优先于 `NOVI_LLM_BASE_URL`、`NOVI_LLM_API_KEY`、`NOVI_LLM_MODEL` 旧配置。启用 Web Provider 后，LangGraph.js 会根据生成提示词在 Workflow、ReAct、Plan & Execute 和 Supervisor 之间选择有界执行模式；控制节点可在运行中重新调度模式。每次 Generate 都按 `Goal → Reference Discovery → Research → Knowledge → Writing → Review → LLM Wiki Finalizer` 协作：Goal Agent 先把问题变成专家级目标和四个领域角色，Reference Discovery 再使用 Goal 查询论文、GitHub 与 Web/文档，Knowledge/Writing 分别产出知识体系和体系文档，Finalizer 汇编最终 LLM Wiki；四个 specialist 仍只能修改各自字段白名单。异步 Job 和工作区先显示 Goal，再显示各 facet 参考检索状态及当前模式、阶段和进度；阶段输出未达到领域质量契约时会在同一运行中重试，达到尝试上限后才保留受控 fallback。默认生成预算为 24 次工具调用、16 次 specialist 执行、单一职责最多 3 次尝试；对话同样默认允许 24 次工具调用。预算可用 `NOVI_AGENT_MAX_TOOL_CALLS`、`NOVI_AGENT_MAX_STAGE_RUNS`、`NOVI_AGENT_MAX_STAGE_ATTEMPTS`、`NOVI_CHAT_MAX_TOOL_CALLS` 覆盖，且仍受 6–64 次工具、32–512 图递归和阶段硬上限约束；实际预算会写入 Artifact runtime，便于评估成本与完成率。未保存 Web Provider 时，旧环境变量仍走原有单次 OpenAI-compatible 网关；两者都未配置时完全离线运行。`GET /api/agent/modes` 返回可用模式，生成请求可传 `{prompt,mode,language}`，其中 `mode=auto` 为默认。

创建 Workspace 时可选择 Wiki 内容语言，默认是简体中文；当前支持 `zh-CN`、`en`、`ja`、`ko`、`fr`、`de`、`es` 和 `pt-BR`，每次 Generate 也可覆盖项目默认值。这里的 i18n 指生成内容语言，Novi 操作界面本身仍以英文为主。设置 `NOVI_LIVE_SOURCES=true` 才会执行真实参考检索，并继续受来源额度、URL 验证和 evidence 门禁约束；未开启时 Reference Discovery 明确记录 `offline`，不会把离线搜索入口描述为已检索或已核验的证据。启用后默认按 landscape、foundations、implementation、evaluation、risks 五个 facet 分别检索，可用 `NOVI_AGENT_RESEARCH_QUERIES` 在 3–8 个 facet 间调整；单个 facet 失败记为 `partial`，全部失败才退款并记为 `fallback`。模型只能用受控来源包中的 `[S1]`、`[S2]` 等标记建立 claim evidence，没有显式标记的主张保持 `unverified`。

每个新项目同时创建一个持久 Agent Session，并直接打开对话式工作区。左栏可新建、切换和删除空闲 Session；中栏保存用户/助手消息，composer 可选择 Auto、Workflow、ReAct、Plan & Execute 或 Supervisor，并设置 Wiki language；`Generate now` 和 composer 都在当前 Session 启动任务。运行中的聊天时间线实时显示 Agent 路由/阶段、发给 LLM 的有界请求、LLM 原始回复、参考检索、工具输入输出和最终 Artifact 保存事件；完成或失败后，同一时间线固化在对应助手消息中，可逐项展开审查。每次运行最多保留 100 条事件，每个请求、回复或工具详情最多约 12000 字符；Provider API Key 会在进入事件前替换为 `[redacted]`，但请求可能包含当前 Workspace 草稿和检索片段，因此事件沿用 Session 的租户/项目访问控制。右栏通过 Files、LLM Wiki、Document 切换工作区文件、不可变成果与文档片段，原有版本比较、导出、知识导入和来源更新仍可使用。每个完成的 Artifact 在 Files 中提供 `llm-wiki.md`，点击后在 Document 以转义纯文本安全预览，其内容与 Markdown 下载一致。`GET/POST /api/projects/:id/sessions` 可列出或新建会话，`GET/DELETE /api/projects/:id/sessions/:sessionId` 可读取或删除空闲会话；运行中的会话返回 409。同步和异步生成都可传 `{prompt,mode,language,sessionId}`，并把 Goal、参考检索状态、运行模式/阶段/进度、运行事件、失败信息以及最终 Artifact 链接写回 Session。Session 按项目和租户双重隔离，随工作空间或账户删除，并包含在账户导出和 v3 备份中；服务重启会把中断 Session 置回 idle、写入失败消息并按原规则退款。
...
`terminal` 和 `exec` 同样默认关闭，除管理员在 Tools 中启用外，服务进程还必须设置 `NOVI_AGENT_EXEC_ENABLED=true`。每次调用将当前项目的虚拟文件树复制到独立临时目录：`terminal` 运行受 `NOVI_AGENT_EXEC_ALLOWED` 约束的一次性 shell 命令，`exec` 使用无 shell 的 program 与 JSON 字符串参数数组；默认 10 秒、最多 30 秒，stdout+stderr 最多 64 KB，最多回写 500 个、总计 2 MB、单个 200 KB 的纯文本文件，符号链接和二进制文件拒绝持久化。子进程只获得最小环境，不继承 Provider、支付或其他应用密钥。该边界不是容器、VM、seccomp 或网络沙箱；生产环境应保持关闭，或只在额外的 OS/容器隔离执行节点中显式启用。
...
外部 ChatGPT 等工具可以帮助用户整理主题，但那一步不属于 Novi；Novi 接收到的最终提示词仍只是未批准的研究请求，必须由第一步 Research Intake Agent 自己判断完整性。

Workspace Composer 是累积式研究入口：发送每一轮消息都会调用 `POST /api/projects/:id/sessions/:sessionId/messages`。新主题的第一轮先进入 Research Intake Agent，而不是立即搜索或写文档。Intake 会判断问题、领域、范围、深度、交付物、语言和边界是否完整；不完整时在 Session 中多轮追问并给出最多五个方向选项，不扣额度、不创建 Job。完整时先展示用户可审阅的研究方法、论文/官方文档/GitHub/Web 来源策略、Goal → Reference → Research → Knowledge → Writing → Review → Finalizer 阶段计划和完成判据；只有用户回复“确认生成”才开始执行。确认后的每轮以最新 Artifact 为编辑基线，合并本轮新增权威来源，创建新的不可变 Artifact 与 `llm-wiki.md`，并把 7 份 Markdown 以 `agent-wiki` 文档索引回 Workspace knowledge。重复内容按内容哈希复用知识文档。Composer 必须先在当前 Web/桌面实例中配置并激活 LLM Provider，且每轮消耗一次生成额度；`NOVI_LIVE_SOURCES=true` 时还消耗一次来源查询额度并检索论文、GitHub 和 Web。`Generate now` 也支持同一 Intake 边界（已有 Artifact 的后续生成直接视为已确认）；`Generate asset` 和 `Regenerate` 仍可直接创建 Knowledge/Research/Paper Artifact。Web 的 `data/novi.json` 与 Electron OS `userData/novi.json` 是独立状态，Provider 必须在实际使用的实例内 `Save & test`。
...
API Key 以 AES-256-GCM 加密保存在租户状态中，API、账户导出和浏览器都不会收到明文或密文。生产必须通过 Secret Manager 设置稳定的 `NOVI_CONFIG_ENCRYPTION_KEY`（至少 32 字符）；本地开发会在数据文件旁生成权限为 0600 的 `data/.novi-config-key`。已知 Provider 使用固定官方 endpoint；Ollama 只允许回环地址；远端自定义 OpenAI-compatible 主机必须使用 HTTPS 并列入逗号分隔的 `NOVI_LLM_ALLOWED_HOSTS`。LangGraph 阶段使用 Provider 的流式响应：Session 时间线会先显示 `streaming` 增量，完整文本聚合后仍必须通过当前阶段 JSON/schema 校验，校验通过才提交 Artifact 和 `llm-wiki.md`。`NOVI_LLM_TIMEOUT_MS` 只保护首个有效 token（默认为 180 秒、上限 300 秒），适配 reasoning 模型的长思考；后续分片空闲超时由 `NOVI_LLM_STREAM_IDLE_TIMEOUT_MS` 控制（默认 120 秒、范围 5–300 秒），单阶段总时长由 `NOVI_LLM_STAGE_MAX_MS` 控制（默认 10 分钟、范围 1–30 分钟）。最大输出 token 由 `NOVI_LLM_MAX_OUTPUT_TOKENS` 控制，完整示例见 `.env.example`。
