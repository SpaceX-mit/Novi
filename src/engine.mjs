...
function goalToMarkdown(project, artifact) {
  ...
  const intake = artifact.workflow?.runtime?.researchIntake || {};
  ...
  if (intake.methodology?.length) lines.push('## Confirmed research method', '', ...intake.methodology.map((item) => `- ${item}`), '');
  if (intake.sourcePlan?.length) lines.push('## Confirmed evidence and source plan', '', ...intake.sourcePlan.map((item) => `- ${item}`), '');
  if (intake.stagePlan?.length) lines.push('## Confirmed execution stages', '', ...intake.stagePlan.map((item) => `- **${item.name}:** ${item.purpose}`), '');
  if (intake.completionCriteria?.length) lines.push('## Confirmed completion criteria', '', ...intake.completionCriteria.map((item) => `- ${item}`), '');
