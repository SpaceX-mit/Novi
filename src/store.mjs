...
export class JsonStore {
  ...
  async read() {
    try {
      ...
      state.version = 3;
      state.projects ||= []; state.jobs ||= []; state.users ||= []; state.sessions ||= []; state.agentSessions ||= []; state.agentToolConfigs ||= []; state.mcpServerConfigs ||= []; state.agentSkillConfigs ||= []; state.audit ||= [];
