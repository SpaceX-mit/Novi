...
function validateCollaborativeCandidate(stage, candidate) {
  ...
  if (stage.id === 'finalizer' && candidate.llmWiki && (String(candidate.llmWiki.summary || '').trim().length < 280 || candidate.llmWiki.sections.length < 8 || candidate.llmWiki.sections.some((section) => String(section?.body || '').trim().length < 260 || String(section?.body || '').split(/\n\s*\n/u).filter(Boolean).length < 2))) throw new Error('LLM Wiki synthesis patch does not meet the minimum quality contract');
