...
function validateCollaborativeCandidate(stage, candidate) {
  ...
  if (stage.id === 'finalizer' && candidate.llmWiki && (String(candidate.llmWiki.summary || '').trim().length < 320 || candidate.llmWiki.sections.length < 8 || candidate.llmWiki.sections.some((section) => String(section?.body || '').trim().length < 320 || String(section?.body || '').split(/\n\s*\n/u).filter(Boolean).length < 2))) throw new Error('LLM Wiki synthesis patch does not meet the minimum quality contract');
...
function stageShapeContract(stage) {
  ...
  if (stage.id === 'finalizer') return 'Finalizer response shape is a compact patch: return exactly {"llmWiki":{"title":"...","summary":"...","sections":[{"title":"...","body":"..."}]}}. Return exactly 8 synthesis sections. Every body is measured after JSON parsing with JavaScript String.length: target 400–500 characters and never below 320; write two substantial coherent paragraphs separated by exactly one blank line, each normally 150–240 characters. Do not stop after a short overview. The first paragraph must explain the mechanism and boundary; the second must state a concrete implementation trade-off, failure mode, or validation test. Keep summary 360–900 characters. Use these section subjects in order: runtime architecture comparison; control-loop and state/checkpoint mechanisms; tools, function calling, and MCP; memory and retrieval consistency; approval, permissions, and sandbox boundaries; implementation and operational trade-offs; evaluation, replay, and failure testing; evidence status, limitations, and next research. The synthesis must explicitly discuss memory, approval, and evaluation using those terms or their Chinese equivalents. Do not return documentMap, glossary, nextQuestions, Deep Dive documents, Markdown, or explanatory prose; Novi preserves those previously validated fields and merges this patch before the final quality gate. Do not use literal unescaped double quotes inside text strings.';
  return '';
}
