...
test('engine generates complete artifacts for all product paths', () => {
  ...
  assert.equal(knowledge.documents.length, 7); assert.deepEqual(knowledge.documents.map((document) => document.name), ['llm-wiki.md', '00-goal.md', '01-research-landscape.md', '02-foundations-and-mechanisms.md', '03-system-architecture.md', '04-implementation-and-evaluation.md', '05-risks-and-frontier.md']); assert.equal(knowledge.content.deepDiveDocuments.length, 5); assert.ok(knowledge.content.deepDiveDocuments.every((document) => document.sections.length >= 6));
...
test('Agent conversation turns require a provider and create cumulative Wiki artifacts indexed into workspace knowledge', async (t) => {
  ...
  assert.equal((await (await fetch(`${base}/api/billing`)).json()).usage.generations, 0);
  // Generate now uses the same first-step boundary. Repeating the workspace
  // topic is not consent and must not create a Job or consume quota.
  response = await fetch(`${base}/api/projects/${created.project.id}/generate?async=true`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ prompt: 'Agent capabilities', mode: 'workflow', sessionId: created.session.id, requireIntake: true }) });
  assert.equal(response.status, 200); const topicIntake = await response.json(); assert.equal(topicIntake.ready, false); assert.equal(topicIntake.requiresConfirmation, false); assert.equal(topicIntake.intake.status, 'incomplete');
  assert.equal((await (await fetch(`${base}/api/billing`)).json()).usage.generations, 0);
  ...
  assert.equal(response.status, 200); const intake = await response.json(); assert.equal(intake.ready, true); assert.equal(intake.requiresConfirmation, true); assert.equal(intake.session.messages.at(-1).kind, 'intake'); assert.ok(intake.intake.methodology.length >= 3); assert.ok(intake.intake.sourcePlan.length >= 3); assert.ok(intake.intake.stagePlan.length >= 5); assert.ok(intake.intake.completionCriteria.length >= 3); assert.match(intake.session.messages.at(-1).content, /研究方法/u); assert.match(intake.session.messages.at(-1).content, /证据与来源策略/u);
  ...
  assert.equal(job.status, 'completed'); assert.equal(job.agentStages.length, 7); assert.equal(job.language, 'en'); assert.equal(job.intake.methodology.length >= 3, true);
...
test('verified evidence preserves a bounded text excerpt for citation-aware Agent research', async () => {
  const previousFetch = global.fetch;
  global.fetch = async () => new Response('<html><head><title>Agent Runtime</title></head><body><h1>Durable execution</h1><script>ignore()</script><p>Checkpoint recovery and idempotent tool effects.</p></body></html>', { status: 200, headers: { 'content-type': 'text/html' } });
  try {
    const [source] = await verifyEvidenceSources([{ name: 'Runtime evidence', kind: 'Official Docs', url: 'https://docs.example/runtime', mapped: true }], { skipDns: true });
    assert.equal(source.verification, 'verified'); assert.equal(source.mapped, true); assert.match(source.contentHash, /^[a-f0-9]{64}$/); assert.match(source.excerpt, /Durable execution/); assert.match(source.excerpt, /idempotent tool effects/); assert.doesNotMatch(source.excerpt, /ignore\(\)/u); assert.ok(source.excerpt.length <= 3_000);
